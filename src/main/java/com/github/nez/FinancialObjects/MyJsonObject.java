...
//        this.jsonObject = json.startsWith("[") ? createJsonObject() : createJsonObjectList();
    ...
    public void putArrayIntoSingleObject(){
        ...
        Map mapToBeReturned = array[0].toMap();
        jsonArrayObjectList.forEach(element->{
            Map emap = element.toMap();
            emap.forEach((key,value)->{
                mapToBeReturned.merge(key,value,(k,v)-> value.toString().equalsIgnoreCase(v.toString()) ?  jsonObject.put(key.toString(), value.toString()) : jsonObject.put(key.toString(), v.toString() + "," + value.toString()));
            });
        });
    }
